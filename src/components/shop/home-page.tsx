...
export function HomePage({
  ...
  const [sponsoredByProduct, setSponsoredByProduct] = useState<Record<string, string>>({});
  const { items, totalKes, openCart } = useCartStore();
  const totalItems = items.reduce((s, i) => s + i.quantity, 0);

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api'}/v1/manufacturer/placements/active`)
      .then((res) => (res.ok ? res.json() : { placements: [] }))
      .then((data: { placements?: Array<{ campaignId: string; productId: string }> }) => {
        const map: Record<string, string> = {};
        for (const p of data.placements ?? []) {
          map[p.productId] = p.campaignId;
        }
        setSponsoredByProduct(map);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="min-h-screen bg-oda-ivory">
      ...
      {/* ── Sticky Header ─────────────────────────────────────────────── */}
      <header className="sticky top-0 z-30 bg-white border-b border-[#E8E8E0]">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <Link href="/" className="text-2xl font-extrabold font-plus-jakarta">
                <span className="text-oda-green">Oda</span><span className="text-[#F8C915]">.</span>
              </Link>
              <button className="flex items-center gap-1 text-sm text-oda-charcoal/60 hover:text-oda-charcoal transition-colors">
                <MapPin className="w-4 h-4 text-oda-green" />
                <span className="font-semibold font-plus-jakarta">Nairobi</span>
                <ChevronDown className="w-3.5 h-3.5" />
              </button>
            </div>
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-1.5 bg-oda-mint rounded-full px-3 py-1.5">
                <Zap className="w-3.5 h-3.5 text-oda-green fill-[#198A2E]" />
                <span className="text-xs font-semibold text-oda-green font-plus-jakarta">~30 min</span>
              </div>
              <Link href="/account" className="w-9 h-9 rounded-full bg-white border border-[#E8E8E0] flex items-center justify-center hover:bg-oda-mint transition-colors shadow-sm">
                <User className="w-4 h-4 text-oda-charcoal" />
              </Link>
              <button
                onClick={openCart}
                className="relative w-9 h-9 rounded-full bg-oda-green flex items-center justify-center hover:bg-oda-green-dark transition-colors shadow-sm"
              >
                ...
                {totalItems > 0 && (
                  <motion.span
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="absolute -top-1 -right-1 w-5 h-5 bg-[#F8C915] rounded-full text-[10px] font-bold text-oda-charcoal flex items-center justify-center font-plus-jakarta"
                  >
                    {totalItems}
                  </motion.span>
                )}
          ...
          <motion.div animate={{ scale: searchFocused ? 1.005 : 1 }} className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-oda-charcoal/30" />
            <Link href="/search">
              <div
                className="w-full pl-11 pr-4 py-3 rounded-2xl bg-oda-ivory border border-[#E8E8E0] text-sm text-oda-charcoal/40 font-plus-jakarta cursor-text hover:border-oda-green/30 transition-colors"
        ...
        <div className="border-t border-[#F0F0E8]">
          <div className="flex overflow-x-auto scrollbar-hide" data-cy="category-grid">
            {CATEGORIES.map((cat, i) => (
              <Link
                key={cat.slug}
                href={`/categories/${cat.slug}`}
                className="flex-shrink-0 flex flex-col items-center gap-1 px-4 py-2.5 hover:bg-oda-ivory transition-colors group min-w-[72px]"
              >
                <div
                ...
                <span className="text-[11px] font-semibold text-oda-charcoal/65 group-hover:text-oda-green whitespace-nowrap font-plus-jakarta transition-colors">
                  {cat.name}
                </span>
                <div className="h-0.5 w-0 bg-oda-green group-hover:w-full transition-all rounded-full" />
              </Link>
            ))}
            <Link
              href="/categories"
              className="flex-shrink-0 flex flex-col items-center gap-1 px-4 py-2.5 hover:bg-oda-ivory transition-colors group min-w-[72px]"
            >
              <div className="w-10 h-10 rounded-xl bg-oda-ivory border border-[#E8E8E0] flex items-center justify-center text-lg transition-transform group-hover:scale-105">
                ⋯
              </div>
              <span className="text-[11px] font-semibold text-oda-charcoal/65 group-hover:text-oda-green whitespace-nowrap font-plus-jakarta transition-colors">
                All
      ...
      {/* ── Hero Section ──────────────────────────────────────────────── */}
      <section className="relative overflow-hidden bg-oda-ivory" style={{ minHeight: 220 }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-10 sm:py-14">
          ...
          >
            <p className="text-xs font-bold text-oda-green uppercase tracking-widest mb-2 font-plus-jakarta">
              Delivered in minutes
            </p>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-oda-charcoal leading-tight font-plus-jakarta mb-2">
              Bulk savings,<br />delivered.
            </h1>
            <p className="text-sm text-oda-charcoal/60 font-plus-jakarta mb-5">
              More in your cart. More in your pocket.
            </p>
            <Link
              href="/categories"
              className="inline-flex items-center gap-2 bg-oda-green hover:bg-oda-green-dark text-white font-bold text-sm px-5 py-2.5 rounded-xl transition-colors font-plus-jakarta shadow-sm"
            >
              Shop now
      ...
      <main className="px-4 lg:px-6 pb-10 pt-5">
        ...
        <div className="mb-6">
          <div className="flex gap-3 overflow-x-auto scrollbar-hide -mx-4 px-4 md:overflow-visible md:mx-0 md:px-0 md:grid md:grid-cols-3">
            {banners.map((banner, i) => {
              ...
              const campaignId =
                banner.linkType?.toUpperCase() === 'CAMPAIGN' ? banner.linkId ?? undefined : undefined;
              return (
                <CampaignBannerTile
                  key={banner.id ?? i}
                  banner={banner}
        ...
        <section className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-oda-green" />
              <h2 className="text-xl font-extrabold text-oda-charcoal font-plus-jakarta">Bestsellers</h2>
            </div>
            <Link href="/categories/groceries-kitchen" className="text-sm text-oda-green font-semibold hover:underline font-plus-jakarta flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {products.slice(0, 6).map((product, i) => (
              <ProductCardSimple
                ...
                product={product}
                index={i}
                campaignId={sponsoredByProduct[product.id]}
              />
            ))}
          </div>
        </section>
        ...
        <section className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Zap className="w-4 h-4 text-oda-green fill-[#198A2E]" />
              <h2 className="text-xl font-extrabold text-oda-charcoal font-plus-jakarta">Quick Picks</h2>
            </div>
            <Link href="/search" className="text-sm text-oda-green font-semibold hover:underline font-plus-jakarta flex items-center gap-1">
              See all <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        ...
        <div className="relative overflow-hidden bg-[#1A1A1A] rounded-[22px] p-6 mb-8 ring-1 ring-[#198A2E]/20 shadow-floating">
          <div className="absolute top-0 right-0 w-48 h-48 bg-oda-green/10 rounded-full blur-3xl pointer-events-none" />
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="text-xs font-bold text-oda-green uppercase tracking-widest mb-1 font-plus-jakarta">For businesses</p>
            ...
            <Link
              href="/b2b"
              className="flex-shrink-0 bg-[#F8C915] hover:bg-[#FFD84D] text-oda-charcoal font-bold px-6 py-3 rounded-xl transition-colors font-plus-jakarta text-sm shadow-sm"
            >
              Apply for B2B
            </Link>
          </div>
        ...
        <p className="text-center text-oda-charcoal/40 text-sm py-4 font-plus-jakarta">
      ...
      {/* ── Sticky Cart Bar ──────────────────────────────────────────────── */}
      {totalItems > 0 && (
        <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-0 left-0 right-0 p-4 z-40" data-cy="cart-bar">
          <div className="w-full max-w-md mx-auto flex gap-2">
          <button
            onClick={openCart}
            className="flex-1 flex bg-oda-green text-white rounded-[20px] px-5 py-4 items-center justify-between shadow-floating"
          >
            <div className="bg-[#166B24] rounded-xl px-3 py-1 text-sm font-bold font-plus-jakarta">
              <span data-cy="cart-item-count">{totalItems}</span> item{totalItems !== 1 ? 's' : ''}
            </div>
          ...
          <Link
            ...
            className="flex-shrink-0 bg-[#F8C915] text-oda-charcoal font-bold rounded-[20px] px-5 py-4 shadow-floating font-plus-jakarta self-stretch flex items-center"
          >
            Checkout
          </Link>
          </div>
...
// ── Sponsored banner tile ─────────────────────────────────────────────────────

function CampaignBannerTile({
  banner,
  index,
  bg,
  textColor,
  ...
  useEffect(() => {
    if (campaignId) trackCampaignEvent(campaignId, 'IMPRESSION');
  }, [campaignId]);
  ...
  return (
    ...
    >
      <Link
        href={href}
        onClick={() => {
          if (campaignId) trackCampaignEvent(campaignId, 'CLICK');
        }}
...
function ProductCardSimple({
  ...
  useEffect(() => {
    if (campaignId) {
      trackCampaignEvent(campaignId, 'IMPRESSION', product.id);
    }
  }, [campaignId, product.id]);

  const handleProductClick = () => {
    if (campaignId) trackCampaignEvent(campaignId, 'CLICK', product.id);
  };
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <Link href={productHref} className="block" onClick={handleProductClick}>
      ...
      <div className="p-2.5">
        <Link href={productHref} onClick={handleProductClick}>
          <p className="text-oda-charcoal text-xs font-medium line-clamp-2 min-h-[2.5rem] mb-1.5 hover:text-oda-green transition-colors font-plus-jakarta">
            {product.name}
          </p>
        </Link>
        <div className="flex items-baseline gap-1 mb-2">
          <span className="text-oda-charcoal font-extrabold text-sm font-plus-jakarta">{formatKES(product.priceKes)}</span>
          {product.mrpKes && (
            <span className="text-oda-charcoal/35 text-[10px] line-through font-plus-jakarta">{formatKES(product.mrpKes)}</span>
          )}
        </div>
        {quantity === 0 ? (
          <motion.button
